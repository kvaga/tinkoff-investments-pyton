...
def get_operations_df(client, account_id : str, allShares, allBonds, allETFs, allCurrencies, allFutures) -> Optional[DataFrame]:
        ...
        :return:
        """
        operations_from_year=2020
        # date_time_from=datetime(operations_from_year,10,1)
        # date_time_to=datetime.utcnow()
        date_time_from=datetime(2024,9,1)
        date_time_to=datetime.utcnow() #(2024,10,1)
        print("Getting history of operations of [{}] account id from [{}] to [{}] datetime".format(account_id, date_time_from, date_time_to))

        r: OperationsResponse = client.operations.get_operations(
            account_id=account_id,
            from_=date_time_from,
            to=date_time_to
        )
...
if __name__ == "__main__":
    ...
    run(None, 'Лист номер один', spreadsheetId=os.environ['GOOGLE_SPREADSHEET_ID'])
